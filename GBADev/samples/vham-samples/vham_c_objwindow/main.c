...
int main(void)
{
   ...
   arSprite[1] = ham_CreateObj((void *)&balls_Bitmap[32*32],  // A pointer to the tile data for this object
    ...
    // This will enable a window which will block / let through BG displays at your choice.
    // You can set the x and y coordinates at your choice, and also later resize the window.
    // In fact, the function is fast enough to be used to update the window coordinates
    // during a HBL interrupt, basically allowing you to resize the shape while the screen
    // is built, giving you the ability to make custom shapes.
    // A special case is window no 2, which is the so called Object Window.
    // This window disregards the x and y coordinates altogether, because the window
    // region is determined by the sprites currently on the screen
    // (see ham_CreateObj for information on how to create an ObjWindow compatible sprite).
    // All the sprite pixel data that is not 0x00 (transparent) is considered to be the window region.
    ham_CreateWin(2,        // window number to initialize and turn on. 0 and 1 are the standard windows that can be used for any section of the screen using the x and y coordinates, winno 2 is the OBJ Window (see comments below)
                  30,       // upper left window x coordinate
                  30,       // upper left window y coordinate
                  100,      // lower right x coordinate
                  100,      // lower right y coordinate
                  0x1f,     // specifies which BGs or OBJs should be visible inside the window.
                  0x1f,     // specifies which BGs or OBJs should be visible outside the window.
                  0);       // enable the use for special fx in this window
